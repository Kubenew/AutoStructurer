...
from autostructurer.config import Config
from autostructurer.utils.ids import sha1_id
from autostructurer.utils.paths import ensure_dir
from autostructurer.utils.hash import compute_phash, hamming_distance

from autostructurer.schema_detect import detect_schema
from autostructurer.entity_extract import extract_entities
from autostructurer.memory.contradiction_graph import contradiction_score
from autostructurer.memory.topics import assign_topic, update_centroid

from autostructurer.embed.text_embedder import TextEmbedder
from autostructurer.embed.clip_embedder import CLIPEmbedder
from autostructurer.embed.batch import batched

from autostructurer.pack4bit import pack_4bit
from autostructurer.storage.sqlite_store import SQLiteStore
from autostructurer.index.faiss_index import IVFIndex
from autostructurer.index.merge_rank import merge_max
from autostructurer.search import unpack_vectors_for_search
from autostructurer.storage.tm_export import export_tm
from autostructurer.storage.zip_export import export_zip
...
class AutoStructurerV5:
    ...
    def ingest_file(self, path: str):
        ...
        for i, ch in enumerate(chunks):
            ...
            # add to faiss later (collect ids)
            inserted += 1

            if ch.get("ref_path"):
                clip_jobs.append((cid, ch["ref_path"]))

        # update FAISS text index
    ...
    def search(self, query: str, mode="hybrid", top_k=10):
        ...
        if mode in ["text","hybrid"]:
            ...
            for score, vid in zip(D[0], I[0]):
                ...
                # fetch_vectors returns (vector_id, chunk_id, ...)
                # simplest: brute map
